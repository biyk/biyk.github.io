...
### 3. `d7_time` — копипаста `h24_time`
- Файл: `src/utils/tasks.js:94-99`
- Оба условия `execDate >= dayAgo`, то есть `d7_time` фактически `h24_time/7`.
- Исправление: во втором условии использовать `sevenDaysAgo`.

### 4. `calcExecutions` падает на пустых ячейках
- Файл: `src/utils/tasks.js:136`
- `item.gained_gold.toString()` кидает `TypeError`, если в строке `task_executions` нет колонки `gained_gold` (строка короче шапки).
- Последствие: падают все статистики и расчёт денег (`today_points` → NaN).
- Исправление: защититься `item.gained_gold != null`, невалидные строки пропускать.

### 5. Деньги = NaN при первом запуске
- Файл: `src/utils/tasks.js:267`
...
- Файл: `src/utils/tasks.js:254-256`
...
- Коды-ключи строятся по ПЕРВОЙ колонке (`google.js:310-315`, `codes[e[0]] = i`), а `TodoNew` пишет первой колонкой `task_uuid`. Значит `codes[название]` = `undefined` → `id = NaN` → `addRow()` вместо `updateRow`. Таблица замусоривается дублями при каждом выполнении/покупке.
- Проверить структуру листов; обновлять по фактическому ключу первой колонки.

### 10. Конфликт `data.todos` и `computed todos`
- Файл: `src/components/TodoList.vue:74, 84`
- Одноимённые свойства — в Vue 3 предупреждение, одно игнорируется. Список обновляется только через ручные `this.todos = $store.getters[...]` (строки 194, 303).
- Исправление: оставить один computed, обновления через store-реактивность.
...
- Файл: `dnd/static/js/db/google.js:183-197, 122-126`
- `updateRow` вызывает `this.waitSending()` без `await`, а флаг `sending` на инстансе `Table`, но каждый вызов делает `new Table(...)`.
...
2. **5, 4** — NaN-деньги / падение статистики
3. **7** — двойные деньги
