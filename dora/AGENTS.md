...
- **Row 1 is always the header `key | value`** — data rows start at row 2. Reads for import go through `_fetchPlanData()` (`values.get` on whole `dora!A:B`). **Writes are strictly row-targeted**: `_readGridRows()` reads real row positions via `spreadsheets.get` + `includeGridData` (because `values.get` *skips empty grid rows*, so its indices can shift), then `_writePlanRow()` writes ONLY the active plan's `B{row}` (`values.update`) or appends a new `[name, json]` row if the plan is absent; `_renamePlanRow()` touches only the plan's `A{row}`; `_deletePlanRow()` clears only its `A{row}:B{row}`. **Never rewrite the whole `A1:B` block, never use `deleteDimension`, never infer row index from `values.get` results** — that bug zeroed another plan's row.
...
- **Auth auto-renewal**: `_renewAuth()` silently refreshes an expired access token via `tokenClient.requestAccessToken({prompt:''})` (90s timeout settles blocked popups). `_doSheetsExport` renews before writing and retries once on 401/403 (`utils.isAuthError`). On failure it sets `dora_unsynced=1` and shows the «☁ Восстановить синхронизацию» banner — its click is a user gesture, so the popup is not blocked. Success hides the banner and clears the flag.
- **All token requests go through ONE gate** (`_tokenRequest(prompt)` in `app.js`): silent renewal (`_renewAuth`) and explicit consent (`_consentAuth`) share the same `tokenClient.callback` and are strictly serialized (`_tokenReq`). Two concurrent `requestAccessToken` calls make GIS kill the popup instantly (the «окно появилось и пропало» bug). Consent calls `abort()` on any in-flight silent request so the popup opens under the user's gesture; the `doAuth` pulse skips while `_tokenReq` is set. **Never open consent via `authorize_button.click()` from app code** (that used to `location.reload()` and produce zero feedback); instead use `_consentAuth()`.
- **Visible auth state** (user must always know if logged in): `_refreshAuthButton` = «Выйти»/«Войти», `_renderSyncStatus` (#sync-status pill: `● email`/`○ синхр. выкл`, email cached in `gapi_user_email` from `/oauth2/v3/userinfo` on consent/boot), and `_toast(msg, isError)` for every auth outcome. No silent reload after consent — success imports the cloud row in place and hides the banner.
- **No true offline refresh token** in this pure client-side flow: `google.accounts.oauth2.initTokenClient` has NO `access_type` and cannot mint a long-lived refresh token for a static site. Renewal is in-browser silent refresh (`{prompt:''}`) — works while Google's browser session / partitioned session cookie is alive. A backend (Authorization Code + client_secret exchange) would be required for refresh tokens that work independently.
- **State kept for silent renewal**: the consent callback persists `gapi_token` from the actual `resp` (`gapi.client.setToken(resp)`), `gapi_token_expires`, and — when the token response carries `refresh_token`/`refresh_expires_in` — `gapi_token_refresh`/`gapi_token_refresh_exp`; these refresh fields are **never overwritten by later silent renewals that lack a refresh_token**. `GoogleSheetDB.hasRefreshSession()` = fresh access token OR unexpired refresh marker; `_refreshAuthButton`/`_handleGDriveAuth`/sync-banner use it to prefer silent renewal over a consent popup. Never persist a token from `gapi.client.getToken()` without re-setting it from `resp`, and never touch `this.authorize_button` as if `this` were the GoogleSheetDB (inside the token-client callback `this` is the token client).
- **Cloud is the source of truth** (never merge): on launch/`doAuth`/switch, `_doSheetsImport` → `_importActiveRow` **overwrites** the local copy of the active plan with the cloud row; on `data:changed` (debounced), `_doSheetsExport` **overwrites** ONLY the cloud row of the active plan with the local plan. No merge, no union, no per-device base snapshot. This is deliberate: staged merge-sync caused objects to resurrect after deletion and demo data to leak into/from the cloud, so it was reverted.
- **Conflict guard on export** (`_doSheetsExport`): a local state that is not a valid plan doc (`_isPlanDoc`) is never pushed; and an **empty local plan never overwrites a non-empty cloud row** (`_isPlanEmpty`) — instead the cloud row is re-imported locally and `dora_unsynced` is cleared (cloud wins). Consequence: to deliberately empty a plan in the DB you must clear/edit its row in the spreadsheet, not delete rooms in the editor.
- `dora_unsynced` is set only when a *push* fails (banner shown) and is cleared after a successful import/export.
- **Stale access tokens are not restored**: `initializeGapiClient` only puts a stored token on `gapi.client` while it is still valid; an expired one is left unset so boot never hits the API with a dead token (401). The `doAuth` timer (10s pulse in `GoogleSheetDB`, dispatched at most once/min) silently renews instead, showing the sync banner when the silent renewal fails. The constructor no longer wipes `gapi_token` when it merely expires — `_renewAuth`/`hasRefreshSession` decide.
...
- Rename updates column A of the matching data row (`_renamePlanRow`); delete clears the row only after content validation (non-plan payloads are kept and skipped, warning logged) — both via `_deletePlanRow`/grid positions, never whole-block rewrites.
